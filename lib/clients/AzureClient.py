...
class AzureClient(BaseClient):
    ...
    def get_snapshot(self, snapshot_name):
        try:
            ...
            return Snapshot(snapshot.name, snapshot.disk_size_gb, snapshot.start_time, snapshot.provisioning_state)
        except Exception as error:
            self.logger.error(
    ...
    def _create_snapshot(self, volume_id):
        ...
        try:
            ...
            snapshot = Snapshot(
                snapshot_info.name, snapshot_info.disk_size_gb, snapshot_info.start_time, snapshot_info.provisioning_state)
